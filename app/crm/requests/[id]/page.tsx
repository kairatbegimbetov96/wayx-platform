...
export default function RequestDetails() {
  ...
  const acceptBid = async (bid: any) => {
    if (!uid || uid !== item.authorUid)
      return window.toast?.('Только автор заявки может принимать', 'error');
    await createDeal({
      requestId: item.id,
      clientUid: uid,
      supplierUid: bid.supplierUid,
  ...
  return (
    <div className="container max-w-5xl py-8">
      ...
      <div className="opacity-70 mb-3">
        {item.mode?.toUpperCase()} • {item.route?.fromCity} → {item.route?.toCity}
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="rounded-2xl border p-4">
          ...
          <div className="space-y-2">
            ...
            {bids.length === 0 && (
              <div className="opacity-60">Пока нет ставок</div>
            )}
          </div>
        </div>
      </div>
        ...
        <div className="space-y-2 text-sm opacity-80">
          ...
          {bids.map((b) => (
            <button
              key={'chat-' + b.id}
              onClick={() => {
                localStorage.setItem('chatSupplierUid', b.supplierUid);
                location.assign(`/crm/requests/${item.id}/chat`);
              }}
              className="px-3 py-2 rounded-lg border hover:bg-slate-100 dark:hover:bg-slate-800 w-full text-left"
            >
      ...
      <style jsx>{`
        .input {
          @apply px-3 py-2 rounded-xl bg-white/60 dark:bg-slate-900/60 border border-slate-200/60 dark:border-slate-800/60 outline-none;
        }
        .btn-primary {
          @apply px-4 py-2 rounded-xl bg-sky-600 hover:bg-sky-700 text-white font-semibold shadow;
        }
      `}</style>
    </div>
  );
